...
int main(void) {
    ...
    printf("float_val,fixed_val,float_cos,fixed_cos,scaled_float,fixed_err\n");
    ...
    while(q16_angle_num < 65535){
        ...
        /* calculate the sine */
        float float_cos = cos(float_val);
        q15_t fixed_cos = q15_cos(q16_angle_num);
        
        /* scale the floating-point to fixed-point for comparison */
        float scaled_cos = (float_cos * 32768.0);
        
        /* calculate the error in points */
        float fixed_err = scaled_cos - (float)fixed_cos;
