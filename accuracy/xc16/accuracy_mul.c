...
int main(void) {
    ...
    printf("float_val,fixed_val,float_mul,fixed_mul,scaled_float,fixed_err\n");
    ...
    while(q15_num1 < 32767){
        ...
        /* calculate the sine */
        float float_mul = float_val * (float)q15_num0/32768.0;
        q15_t fixed_mul = q15_mul(q15_num1, q15_num0);
        
        /* scale the floating-point to fixed-point for comparison */
        float scaled_sine = (float_mul * 32768.0);
        
        /* calculate the error in points */
        float fixed_err = scaled_sine - (float)fixed_mul;
